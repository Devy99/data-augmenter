...
class Styleformer():
  ...
  def __init__(
    ...
    import transformations.text.utils.formality_change.adequacy as Adequacy

    self.style = style
    self.adequacy = adequacy_model_tag and Adequacy.Adequacy(model_tag=adequacy_model_tag)
    self.model_loaded = False
    
    if self.style == 0:
      cache_file = ctf_model_tag.replace('/', '-')
      ...
      self.ctf_tokenizer = tokenizer
      self.ctf_model = model
      #print("Casual to Formal model loaded...")
      self.model_loaded = True
    elif self.style == 1:
      cache_file = ftc_model_tag.replace('/', '-')
      model = cache_helper.get_file(f"{cache_file}-model")
      if model is None:
      ...
      self.ftc_tokenizer = tokenizer
      self.ftc_model = model
