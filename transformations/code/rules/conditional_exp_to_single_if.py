...
class ConditionalExpToSingleIf(Transformation):
    """
    Code transformation rule from the SPAT tool
    "Change a conditional expression statement into a single if statement"
    
    Code reference: https://github.com/Santiago-Yu/SPAT/blob/master/src/spat/rules/ConditionalExp2SingleIF.java
    """
    
    def generate(self, sentence: str):
        sentence, block_line, start_char, end_char = extract_code_specs(sentence, '<START>', '<END>')
        output = subprocess.run(['java', '-jar', SPAT_JAR_PATH, sentence, 'ConditionalExp2SingleIF', str(self.max_outputs),
